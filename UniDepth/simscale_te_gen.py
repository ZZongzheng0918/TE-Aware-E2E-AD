...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Estimate SimScale front-camera depth and project 2D traffic elements to 3D."
    )
    parser.add_argument("--log-dir", type=Path, required=True, help="Path to the NAVSIM log directory.")
    parser.add_argument("--data-root", type=Path, required=True, help="Path to the NAVSIM sensor_blobs directory.")
    parser.add_argument("--det-dir", type=Path, required=True, help="Path to SimScale YOLO prediction outputs.")
...
def process_frame(model, frame, args, split):
    import numpy as np
    ...
    scene_name = image_path.parent.parent.name
    label_path = args.det_dir / split / scene_name / "labels" / f"{image_token}.txt"
    save_path = args.save_dir / split / scene_name / f"{image_token}.json"
    if not label_path.exists():
    ...
    write_json(save_path, traffic_elements)
...
def process_log(model, log_path, args, split):
    ...
    for frame in log:
        process_frame(model, frame, args, split)
...
def main():
    ...
    for split in args.splits:
        ...
        if not split_log_dir.is_dir():
            raise FileNotFoundError(f"SimScale log split directory not found: {split_log_dir}")
